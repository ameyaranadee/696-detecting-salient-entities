...
with open('./spacy_alias_trf_recall.json') as file:
    trf_alias_json = json.load(file)

with open('./spacy_no_alias_trf_recall.json') as file:
    trf_no_alias_json = json.load(file)

with open('./spacy_alias_sm_recall.json') as file:
    sm_alias_json = json.load(file)


with open('./spacy_no_alias_sm_recall.json') as file:
...
plt.title('Recall of salient entities vs fuzzy matching ratio')
