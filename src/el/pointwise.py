...
def construct_pointwise_prompt(entity, candidate):
    return [{"role": "user", "content": f"""
        You are an entity disambiguator...
        entity: {entity['entity_title']}
        context: {entity['surrounding_context']}
        candidate: {candidate['title']}. {candidate['text'][:300]}
        Keep the answer to exactly one compact JSON object: {{\"final_decision\":\"yes|no\",\"reasoning\":\"\"}}"""}]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/pointwise", help="Directory to store intermediate and final results")
